...
with tab_measure:
    ...
    with c1:
        ...
        start_i = st.number_input("Start Current (A)", format="%.1e", min_value=1e-7, max_value=0.1, key='sweep_start_i', value=max(1e-7, float(st.session_state.sweep_start_i)), on_change=sync_setting, args=("sweep_start_i", "sweep_start"))
        stop_i = st.number_input("Stop Current (A)", format="%.1e", min_value=1e-7, max_value=0.1, key='sweep_stop_i', value=max(1e-7, float(st.session_state.sweep_stop_i)), on_change=sync_setting, args=("sweep_stop_i", "sweep_stop"))
        steps = st.number_input("Steps", min_value=2, max_value=50, key='sweep_steps', value=max(2, int(st.session_state.sweep_steps)), on_change=sync_setting, args=("sweep_steps", "sweep_steps"))
        st.divider()
        freq = st.number_input("Frequency (Hz)", min_value=0.1, max_value=100000.0, key='sweep_freq', value=max(0.1, float(st.session_state.sweep_freq)), on_change=sync_setting, args=("sweep_freq", "sweep_freq"))
